...
kable(tab01,table_format,booktabs = T, linesep = "",col.names = cnames, caption = cap1, label = "ola") %>%
  kable_styling(
    full_width = T,
...
De todas estas plantillas veamos el detalles de tres de ellas: OSF Preregistration, Secondary Data Preregistration y Preregistration Template from AsPredicted.org.

Una vez ecojamos una plantilla, la página se verá como la Figura N°  \@ref(fig:osfprereg5). Esta sección es transversal a todas las plantillas y consiste en el registro de los metadatos. Específicamente, OSF nos solicitará que registremos:
...
- Áreas discplinares, como _Arts and Humanities_, _Business_, _Social and Behavioral Sciences_ entre otras.
...
- Información del estudio: Principalmente solicita listar de forma concisa y detallada las hiótesis que serán puestas a prueba. Se solicita reportar si la hipótesis es direccional o no, y en el caso de serlo, especificar el sentido de la dirección. Si el estudio incluye moderaciones o mediaciones, plantearlas en hipótesis apaerte.

- Plan del diseño: En esta sección se hacen cuatro preguntas importantes. La primera es sobre el **Tipo del estudio**, donde se solicita especificar si el estudio es de tipo experimental, observacional, un metanálisis u otro. En segundo lugar, esta el **_Blinding_**, el cual trata sobre quienes están en conocimiento de las condiciones de tratamiento en el caso de un diseño experimental. En el caso de un diseño experimental no aplica. En tercer lugar, se pregunta sobre el **Diseño del estudio**, donde se solicita la mayor cantidad de detalles posibles respecto a este tópico. Por último, se solicita especificar cuál será el tipo de **Randomización** (en el caso de un diseño experimental).
...
Para elaborar un preregistro debemos hacer click en el rectangulo azul que dice _Create_. Una vez hecho eso, nos pedirá una dirección de email para continuar. Cuando ingresemos un email, nos enviará un link al email que hayamos ingresado, con ese link podremos comenzar el preregistro. Una vez hayamos entrado en el link, veremos la plantilla de preregistro. Lo primero que aparece es una sección donde debemos escribir los email de los autores colaboradores del estudio. También, nos da la opción de añadir otros email además del que hemos introducido. Una vez pasada esta parte, ya nos encontramos con las preguntas del preregistro, las cuales son las siguientes:
...
Las preguntas son bastante autoexplicativas, pero no está de más entregar algunos detalles adicionales. En la pregunta de recolección de datos, las opciones son tres: "Sí, se han recolectado datos", "No, no se han recolectado datos" y "Es complicado". Es importante mencionar que en esta plantilla, la respuesta de que se han recolectado datos no es valida, por lo que si se está llevando a cabo un estudio con datos secundarios hay responder "Es complicado" y en la pregunta 8 de la plantilla especificar por qué este preregistro sigue siendo válido pese a que los datos son pre-existentes. Otro detalle importante es que cada pregunta está pensada para ser respuesta en aproximadamente una oración. Esta plantilla tiene el objetivo de ser lo más eficiente posible, por lo que, en general, se recomienda que todo el documento no pase de los 3200 caracteres. Otro detalle a especificar es que la pregunta acerca del tipo de estudio que se está preregistrando también es semicerrada, tenemos las opciones de: "Proyecto de clase", "Experimento", "Encuesta", "Estudio observacional" y "Otro". Es responsabilidad de quien hace el preregistro el seleccionar la opción que más se asemeje a su situación. Por último, es importante señalar que el preregistro, al menos en la página de AsPredicted, solo puede ser rellenado en inglés. En caso de utilizar otro idioma solicitará traducirlo.
